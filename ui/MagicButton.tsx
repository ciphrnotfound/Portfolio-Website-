...
const MagicButton = ({title, icon, position, handleClick, otherClasses}:{ title: string, icon: React.ReactNode, position: string, handleClick?: () => void, otherClasses: string; }) => {
    return(
        <>
            <button className="relative inline-flex h-12 overflow-hidden rounded-2xl md:w-60 md:mt-10 p-[1px] p-[1px] focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2 focus:ring-offset-slate-50" onClick={handleClick}>
